  ...
  module Event
    ...
    module ClassMethods
      def embeds_many_occurrences(options = {})
        ...
        scope :occurs_until, ->(date_time, dtend_field = :_dtend) { MongoidOccurrenceViews::Queries::OccursUntil.criteria(self, date_time, dtend_field: dtend_field) }

        scope :order_by_start, ->(order = :asc, dtstart_field = :_dtstart) { MongoidOccurrenceViews::Queries::OrderByStart.criteria(self, order, dtstart_field: dtstart_field) }
        scope :order_by_end, ->(order = :asc, dtend_field = :_dtend) { MongoidOccurrenceViews::Queries::OrderByEnd.criteria(self, order, dtend_field: dtend_field) }
